...
import DecryptedText from "../components/Shared/DecryptedText"; // keep if you have this
...
// -------------------------
// HAPUS Bagian ThemeContext Lokal
// -------------------------
// import { ThemeContext } from "../context/ThemeContext";
// (Bagian ini sudah cukup, tidak perlu buat createContext lagi)
...
export default function WebsiteSecurityScanner() {
  ...
  async function handleScan() {
    ...
    try {
const res = await fetch("https://your-backend.vercel.app/api/vt/scan", {
        method: "POST",
      ...
      console.error(err);
      setStatus("Kesalahan saat submit URL. Periksa backend / API key.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function pollResult(id) {
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      try {
const res = await fetch(`https://your-backend.vercel.app/api/vt/result/${id}`);
        if (res.ok) {
          const data = await res.json();
      ...
      <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
        <div>
          ...
          <h2 className="text-2xl font-semibold mt-1">
            {result?.data?.attributes?.content?.title ||
              result?.data?.id}
          </h2>
          <div
        ...
        <div className="md:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="p-3 rounded-xl bg-yellow-900/30 text-center">
            <div className="text-2xl font-bold text-yellow-400">
              {stats.suspicious ?? "N/A"}
            </div>
            <div className="text-sm text-zinc-300">Suspicious</div>
          </div>

          <div className="p-3 rounded-xl bg-red-900/30 text-center">
            <div className="text-2xl font-bold text-red-400">
              {stats.malicious ?? "N/A"}
            </div>
            <div className="text-sm text-zinc-300">Malicious</div>
          </div>

          <div className="p-3 rounded-xl bg-gray-700/30 text-center">
            <div className="text-2xl font-bold text-gray-300">
              {stats.undetected ?? 0}
            </div>
            <div className="text-sm text-zinc-300">Undetected</div>
          </div>
        </div>
