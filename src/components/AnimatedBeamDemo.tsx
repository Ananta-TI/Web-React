...
const Circle = forwardRef<HTMLDivElement, { className?: string; children?: React.ReactNode }>(({ className, children }, ref) => {
  return (
    <div ref={ref} className={cn("z-10 flex size-16 sm:size-22 items-center justify-center p-3", className)}>
      {children}
    </div>
  );
});

// Hapus useContext. Ekspor komponen yang sudah dibungkus React.memo
export const AnimatedBeamDemo = React.memo(function AnimatedBeamDemo({ isDarkMode = true }: { isDarkMode?: boolean }) {
  ...
  return (
    <div
      className={cn(
        "relative flex h-[600px] w-full items-center justify-center overflow-hidden px-4 sm:px-10 rounded-2xl",
        isDarkMode ? "bg-zinc-900 " : "bg-transparent " // Tambahkan 'bg-' pada gray
      )}
      ref={containerRef}
    >
      <div className="flex size-full max-h-[500px] max-w-7xl flex-col items-stretch justify-between gap-6 md:gap-10">
        ...
        <div className="flex flex-row items-center justify-between">
          <Circle ref={div8Ref}><Icons.gsap isDarkMode={isDarkMode} /></Circle>
          <Circle ref={div9Ref}><Icons.framerMotion isDarkMode={isDarkMode} /></Circle>
        </div>

        {/* Row 2 */}
        <div className="flex flex-row items-center justify-between">
          <Circle ref={div1Ref}><Icons.html isDarkMode={isDarkMode} /></Circle>
          <Circle ref={div5Ref}><Icons.css isDarkMode={isDarkMode} /></Circle>
        </div>

        {/* Row 3 (Center) */}
        <div className="flex flex-row items-center justify-between">
          <Circle ref={div2Ref}><Icons.js isDarkMode={isDarkMode} /></Circle>
          
          <Circle
            ref={div4Ref}
            className={`cursor-target rounded-2xl w-[140px] h-[70px] sm:w-[280px] sm:h-[120px] transition-all duration-300 ${isDarkMode ? "bg-zinc-800" : "bg-zinc-200"}`}
          >
            <span
              className={`whitespace-pre-wrap bg-clip-text text-center text-3xl sm:text-6xl md:text-8xl font-semibold font-lyrae leading-tight text-transparent ${
                isDarkMode ? "bg-gradient-to-b from-zinc-100 to-zinc-900" : "bg-gradient-to-b from-zinc-800 to-zinc-100"
              }`}
            >
              Skills
            </span>
          ...
          <Circle ref={div6Ref}><Icons.react isDarkMode={isDarkMode} /></Circle>
        </div>

        {/* Row 4 */}
        <div className="flex flex-row items-center justify-between">
          <Circle ref={div3Ref}><Icons.tailwind isDarkMode={isDarkMode} /></Circle>
          <Circle ref={div7Ref}><Icons.laravel isDarkMode={isDarkMode} /></Circle>
        </div>
      </div>
