...
const SteampunkWatch = () => {
  ...
  const handMinRef = useRef(null);
  const handHrRef = useRef(null);

  useEffect(() => {
    if (typeof window === 'undefined') return;

    const startAnimation = () => {
      const face01 = document.querySelector("#face01")?.getAttribute("d") || "";
      const face02 = document.querySelector("#face02")?.getAttribute("d") || "";
      const handSec01 = document.querySelector("#handSec01")?.getAttribute("d") || "";
      const handSec02 = document.querySelector("#handSec02")?.getAttribute("d") || "";
      const handMin01 = document.querySelector("#handMin01")?.getAttribute("d") || "";
      const handMin02 = document.querySelector("#handMin02")?.getAttribute("d") || "";
      const handHr01 = document.querySelector("#handHr01")?.getAttribute("d") || "";
      const handHr02 = document.querySelector("#handHr02")?.getAttribute("d") || "";

      gsap.set(faceRef.current, { attr: { d: face01 } });
      gsap.set(handSecRef.current, { attr: { d: handSec01 } });
      gsap.set(handMinRef.current, { attr: { d: handMin01 } });
      gsap.set(handHrRef.current, { attr: { d: handHr01 } });

      setTimeSec();
      setTimeMinHr();
      
      gsap.set([wrapperRef.current, ".vline"], { autoAlpha: 1 });
      ...
      gsap.to(minRef.current, 0.5, { 
        rotation: getMinRotation, 
        transformOrigin: "50% 50%", 
        ease: "none", 
        onComplete: function() {
          if (gsap.getProperty(minRef.current, "rotation") >= 360) {
            gsap.set(minRef.current, { rotation: 0, transformOrigin: "50% 50%" });
          }
      ...
      // Hour hand animation
      gsap.to(hrRef.current, 0.5, { 
        rotation: getHrRotation, 
        transformOrigin: "50% 50%", 
        ease: "none", 
        onComplete: function() {
          if (gsap.getProperty(hrRef.current, "rotation") >= 360) {
            gsap.set(hrRef.current, { rotation: 0, transformOrigin: "50% 50%" });
          }
          this.invalidate().delay(5).restart(true);
        } 
      });
      ...
      gsap.to(secRef.current, 0.5, { 
        rotation: getSecRotation, 
        transformOrigin: "50% 50%", 
        ease: "bounce", 
        onComplete: function() {
          setTimeSec();
          if (gsap.getProperty(secRef.current, "rotation") >= 360) {
            gsap.set(secRef.current, { rotation: 0, transformOrigin: "50% 50%" });
          }
      ...
      // Second hand morph animation
      const tg1 = gsap.timeline({ 
        repeat: -1, 
        repeatDelay: 5, 
      ...
      .delay(1)
      .call(() => {
        const rotation = parseFloat(gsap.getProperty(secRef.current, "rotation").toFixed(1));
        if ((rotation > 30 && rotation < 150) || (rotation > 210 && rotation < 330)) {
          gsap.timeline({ repeat: 0, defaults: { duration: 0.25, ease: "bounce.in" } })
          .to(handSecRef.current, { morphSVG: "#handSec02" })
          .to(handSecRef.current, { morphSVG: "#handSec01" });
        }
      })
      .set(secRef.current, { 
        onComplete() {
      ...
      // Minute hand morph animation
      const tg2 = gsap.timeline({ 
        repeat: -1, 
        repeatDelay: 5, 
        defaults: { duration: 1.5, ease: "bounce" } 
      })
      .delay(5)
      .call(() => {
        const rotation = parseFloat(gsap.getProperty(minRef.current, "rotation").toFixed(1));
        if ((rotation > 5 && rotation < 175) || (rotation > 185 && rotation < 355)) {
          gsap.timeline({ repeat: 0, defaults: { duration: 0.25, ease: "bounce.in" } })
          .to(handMinRef.current, { morphSVG: "#handMin02" })
          .to(handMinRef.current, { morphSVG: "#handMin01" });
        }
      })
      .set(minRef.current, { 
        onComplete() {
      ...
      // Hour hand morph animation
      const tg3 = gsap.timeline({ 
        repeat: -1, 
        repeatDelay: 5, 
        defaults: { duration: 1.5, ease: "bounce" } 
      })
      .delay(7)
      .call(() => {
        const rotation = parseFloat(gsap.getProperty(hrRef.current, "rotation").toFixed(1));
        if ((rotation > 2 && rotation < 178) || (rotation > 182 && rotation < 358)) {
          gsap.timeline({ repeat: 0, defaults: { duration: 0.25, ease: "bounce.in" } })
          .to(handHrRef.current, { morphSVG: "#handHr02" })
          .to(handHrRef.current, { morphSVG: "#handHr01" });
        }
      })
      .set(hrRef.current, { 
        onComplete() {
      ...
      // --- Time functions ---
      function setTimeSec() {
        gsap.set(secRef.current, { rotation: getSecRotation, transformOrigin: "50% 50%" });
      }

      function setTimeMinHr() {
        gsap.set(minRef.current, { rotation: getMinRotation, transformOrigin: "50% 50%" });
        gsap.set(hrRef.current, { rotation: getHrRotation, transformOrigin: "50% 50%" });
      }

      function getSecRotation() {
        const seconds = new Date().getSeconds();
        const rotation = seconds * 6;
        const scaleXSec = gsap.getProperty(secRef.current, "scaleX");

        const difference = Math.abs(gsap.getProperty(secRef.current, "rotation") - rotation);
        if (difference >= 12) {
          gsap.set(secRef.current, { rotation, transformOrigin: "50% 50%" });
        }

        if ((rotation >= 180 && rotation < 360) && (scaleXSec === 1)) {
          gsap.to(secRef.current, { scaleX: -1, duration: 0.25 });
      ...
      function getMinRotation() {
        ...
        const rotation = newDateTime.getMinutes() * 6 + newDateTime.getSeconds() * 6 / 59;
        const scaleXMin = gsap.getProperty(minRef.current, "scaleX");

        const difference = Math.abs(gsap.getProperty(minRef.current, "rotation") - rotation);
        if (difference >= 5) {
          gsap.set(minRef.current, { rotation, transformOrigin: "50% 50%" });
        }

        if ((rotation >= 180 && rotation < 360) && (scaleXMin === 1)) {
          gsap.to(minRef.current, { scaleX: -1, duration: 0.25 });
      ...
      function getHrRotation() {
        const newDateTime = new Date();
        const rotation = (newDateTime.getHours() % 12) * 30 + newDateTime.getMinutes() * 0.5;
        const scaleHr = gsap.getProperty(hrRef.current, "scaleX");

        const difference = Math.abs(gsap.getProperty(hrRef.current, "rotation") - rotation);
        if (difference >= 5) {
          gsap.set(hrRef.current, { rotation, transformOrigin: "50% 50%" });
        }

        if ((rotation >= 180 && rotation < 360) && (scaleHr === 1)) {
          gsap.to(hrRef.current, { scaleX: -1, duration: 0.25 });
  ...
  return (
    <div className="w-full h-screen flex justify-center items-center bg-gradient-to-br from-[#a34a01] to-[#654201]">
      <div className="vline fixed top-0 left-1/2 h-1/2 w-0.5 bg-white opacity-50 -z-10 invisible"></div>
      
      <div ref={wrapperRef} className="gsapWrapper invisible">
        <div className="wBody relative aspect-square">
          <div className="watch absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full h-full aspect-square overflow-visible">
            <svg 
              id="watchSVG" 
              overflow="visible" 
              version="1.1" 
            ...
            >
              ...
              <defs>
                <filter id='shadow' x="-5%" y="-5%" width="110%" height="110%">	
                  <feDropShadow dx="0" dy="0" floodColor="#a34a01" stdDeviation="1" />
                </filter>	
                <filter id='shadow2' x="-25%" y="-25%" width="150%" height="150%">	
                  <feDropShadow dx="0" dy="0" floodColor="#a34a01" stdDeviation="1.25" />
                </filter>
